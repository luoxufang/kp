...
//添加背景图
@property (nonatomic, strong) SKSpriteNode *bgImage1;
...
//        _bgImage1 = [[SKSpriteNode alloc] initWithImageNamed:@"bg_1.png"];
//        _bgImage1.position = CGPointMake(self.size.width/2., self.size.height/2.);
//        [self addChild:_bgImage1];
//
//      _bgImage2 = [[SKSpriteNode alloc] initWithImageNamed:@"bg_2.png"];
//      _bgImage2.position = CGPointMake(self.size.width/2.+ self.size.width, self.size.height/2.);
//      [self addChild:_bgImage2];
...
- (void)initGround
{
    self.ground = [[SKSpriteNode alloc] initWithTexture:[SKTexture textureWithImageNamed:@"sceneView_ground.png"] color:[UIColor clearColor] size:CGSizeMake(self.size.width, 170)];
  
    self.ground.position = CGPointMake(self.size.width / 2, CGRectGetMidY(self.frame) - 284);
    if ([[UIScreen mainScreen] bounds].size.width == 768) {
      self.ground.size = CGSizeMake(self.size.width, 125);
      self.ground.position = CGPointMake(self.size.width / 2, CGRectGetMidY(self.frame) - 454);
    }
...
- (void)createTimeLabel
{
    ...
    self.timeLabel.position = CGPointMake(self.size.width / 2, self.size.height / 2 + 190);
...
//    SKSpriteNode *stage = [[SKSpriteNode alloc]initWithColor:[SKColor redColor] size:CGSizeMake(self.frame.size.width * 3, 470)];
  
    SKSpriteNode *stage = [[SKSpriteNode alloc] initWithImageNamed:@"ground"];
    stage.size = CGSizeMake(self.frame.size.width * 3, 420);
    stage.position = CGPointMake(self.size.width, CGRectGetMidY(self.frame) - 158);
    if ([UIScreen mainScreen].bounds.size.width == 768) {
      stage.size = CGSizeMake(self.frame.size.width * 3, 600);
      stage.position = CGPointMake(self.size.width, CGRectGetMidY(self.frame) - 218);
    }
    [self addChild:stage];
    ...
    int duration = 5;
    if ([UIScreen mainScreen].bounds.size.width == 768) {
        stage.size = CGSizeMake(self.size.width * 2, 600);
        duration = 4;
    }
...
- (void)createPlayer
{
    ...
    if ([[UIScreen mainScreen]bounds].size.width == 768) {
        self.player.size = CGSizeMake(80, 110);
    }
    self.player.position = CGPointMake(40, CGRectGetMidY(self.frame) +200);
...
//  SKColor *color = [SKColor colorWithPatternImage:[UIImage imageNamed:@"ground_long"]];
  NSString *imageName = @"ground_long";
  CGFloat height = 16 ;
    if ([[UIScreen mainScreen]bounds].size.width == 768) {
        duration = 1;
//      return;
...
//  self.object = [[SKSpriteNode alloc]initWithColor:color size:CGSizeMake(width, 36)];
  self.object = [[SKSpriteNode alloc] initWithImageNamed:imageName];
  self.object.size = CGSizeMake(width, height);
...
#pragma mark - 背景移动
- (void)bgImageMove{
  _bgImage1.position = CGPointMake(_bgImage1.position.x-1, self.size.height/2.);
  _bgImage2.position = CGPointMake(_bgImage2.position.x - 1, self.size.height/2.);
  
  if (_bgImage1.position.x <= -self.size.width/2. ) {
    _bgImage1.position = CGPointMake(self.size.width/2.+self.size.width, self.size.height/2.);
  }
  if (_bgImage2.position.x<=-self.size.width/2.) {
    _bgImage2.position = CGPointMake(self.size.width/2.+self.size.width, self.size.height/2.);
  }
